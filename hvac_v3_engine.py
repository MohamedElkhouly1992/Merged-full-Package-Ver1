...
def simulate_baseline_no_degradation(
    ...
    for d in range(cfg.years * cfg.days_per_year):
        ...
        daily_rows.append({
            ...
            "people_count": loads.get("people", derived["N_people_max"] * occ),
            "internal_gains_kw": loads.get("internal_kw", derived["Internal_kw_max"] * max(0.20, occ * cfg.INTERNAL_USE_FACTOR + 0.20)),
            "sensible_people_kw": loads.get("people", derived["N_people_max"] * occ) * bldg.sensible_w_per_person / 1000.0,
...
def simulate_combo(
    ...
    for d in range(cfg.years * cfg.days_per_year):
        ...
        daily_rows.append({
            ...
            "internal_gains_kw": derived["Internal_kw_max"] * max(0.20, occ * cfg.INTERNAL_USE_FACTOR + 0.20),
...
def run_scenario_model(
    ...
    if weather_df is not None:
        base_weather = ensure_365_daily_weather(weather_df)
        weather_meta = weather_summary_dict(base_weather, "uploaded_dataframe", None)
    elif weather_mode == "epw" and epw_path:
        base_weather = read_epw_daily(epw_path)
        weather_meta = weather_summary_dict(base_weather, "epw", epw_path)
    elif weather_mode == "csv" and csv_path:
        base_weather = read_weather_csv_daily(csv_path)
        weather_meta = weather_summary_dict(base_weather, "csv", csv_path)
    elif weather_mode == "uploaded" and epw_path:
        base_weather = read_weather_auto_daily(epw_path)
        weather_meta = weather_summary_dict(base_weather, "uploaded", epw_path)
    else:
